...
- [x] 共有モジュール抽出 (terminal-colors.ts 12ファイル, cli-args.ts 9ファイル, mask.ts)
- [x] Playwright ページ再利用 (fix-loop), PNG IHDR ヘッダ読み取り, Gemini SDK 初期化最適化

### Snapshot / URL Compare
- [x] `vrt snapshot` コマンド (URL → multi-viewport キャプチャ + baseline diff)
- [x] `vrt compare --url / --current-url` URL モード (page.goto ベース)
- [x] `--mask` セレクタマスキング (visibility: hidden で動的コンテンツ除外)
- [x] プロジェクト名: vrt-harness → vrt
...
実際のプロジェクトで vrt を使い、実用性を検証する。

- [x] `vrt snapshot` コマンド追加 (URL → 複数 viewport キャプチャ + baseline diff)
- [x] `vrt compare --url` URL モード追加 (page.goto ベース)
- [x] luna.mbt で dogfooding: false positive 率 0% (6 ページ × 2 viewport)
- [x] sol.mbt で dogfooding: false positive 率 20% (root ページの動的コンテンツ)
- [x] 結果を `docs/reports/2026-04-05-dogfood-luna-sol.md` に記録
...
**Status**: #18-22 全て Open (2026-04-05 時点で進捗なし)。crater 側修正待ち。

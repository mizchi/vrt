...
2026-04-02 時点では crater core / BiDi / `vrt` 連携は実装済みで、未完は external stylesheet discovery などの v2 領域。

## 背景

当初の `vrt` は breakpoint を quickcheck の境界値として扱う発想は持っていたが、抽出は regex ベースで、`<style>` しか見ていなかった。
...
そのため、breakpoint discovery の正規化と contract は `crater` 側に寄せ、`vrt` 側はそれを test input に変換する責務に絞るのが自然。

## ゴール

- CSS media query から responsive breakpoint 候補を parser ベースで抽出できる
- `vrt` がそれを quickcheck の境界値として viewport 群に変換できる
...
- viewport 生成は `vrt` の test policy
